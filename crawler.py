...
class InsuranceProduct:
    def __int__(self, product_id, product_name, company, coverage_brief=None, coverage=None, exception=None,
...
def iter_products(company):
    ...
    def _write_product_detail(i):
        p = InsuranceProduct(product_id=candidates[i - 1], product_name=candidates[i], company=company)
        ...
        coverage_li = [i.text.replace(' ', '').replace('\n', '') for i in
                       soup.select("#contentbox > div > table:nth-child(6) > tbody > tr > td:nth-child(2)")][1:]
        coverage_b_li = [i.text.replace(' ', '').replace('\n', '') for i in
                         soup.select("#contentbox > div > table:nth-child(6) > tbody > tr > td:nth-child(1)")][1:]
        exception_li = [i.text.replace(' ', '').replace('\n', '') for i in
                        soup.select("#contentbox > div > table:nth-child(8) > tbody > tr > td:nth-child(2)")][1:]
        exception_b_li = [i.text.replace(' ', '').replace('\n', '') for i in
                          soup.select("#contentbox > div > table:nth-child(8) > tbody > tr > td:nth-child(1)")][1:]
